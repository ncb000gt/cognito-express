...
describe('Strategy Positive Scenarios', () => {
	...
	it('should check if Strategy can be initialized successfully', done => {
		strategy = new Strategy(strategyConfig);
		strategy.init(inited => {
			expect(inited).to.eql(true);
			done();
		});
	});

	it.skip('should check if Validate function can fail successfully when invalid token is passed', done => {
		strategy = new Strategy(strategyConfig);
		strategy.init(async callback => {
			try {
				await strategy.validate(token);
			} catch(err) {
				expect(err.name).to.eql('TokenExpiredError');
				// TODO: should be this commented one, but the token is old af apparently...
